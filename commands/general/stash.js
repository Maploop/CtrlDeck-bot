const { SlashCommandBuilder, EmbedBuilder, ActionRowBuilder, ActionRow, TextInputBuilder, TextInputStyle, SlashCommandSubcommandBuilder } = require('discord.js');
	...
	async execute(interaction) {
        ...
        switch (sub) {
            case "view": {
                ...
                updateCacheNeccessary = !(stashData[authorId]['CACHE']) || (interaction.options.getBoolean('update_cache'));

                entryCount = Object.keys(stashData[authorId]).length;
                embed = new EmbedBuilder()
                    .setColor("Aqua")
                    .setTitle("Stash data for \"" + authorId + "\"")
                    .setDescription("Found `" + entryCount + "` entr" + (entryCount > 1 ? "ies" : "y") + ".");
                ...
                for (key of Object.keys(stashData[authorId])) {
                    ...
                    for (altKey of Object.keys(stashData[authorId][key])) {
                        if (altKey.startsWith("INTERNAL__")) 
                            continue;

                        desc += "• " + altKey + ": " + stashData[authorId][key][altKey] + "\n";
                    }
                    desc += `-> **[[VIEW]](${key})**`
